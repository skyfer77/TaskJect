...
{
	public class SubscriptionExpirationInWeekDomainEventHandler : IDomainEventHandler<SubscriptionExpirationInWeekDomainEvent>
	{
		...
		public SubscriptionExpirationInWeekDomainEventHandler(
		...
		public bool CanHandle(IDomainEvent domainEvent) => domainEvent is SubscriptionExpirationInWeekDomainEvent;

		public async Task HandleAsync(IDomainEvent domainEvent)
		{
			if (domainEvent is not SubscriptionExpirationInWeekDomainEvent subscriptionEvent)
			{
				return;
			}
			...
			var notification = new SystemNotification
			{
				...
				Title = _localizer["SubscriptionExpiresInWeekTitle"],
				Message = _localizer["SubscriptionExpiresInWeekMessage"]
			};
			...
			if (!string.IsNullOrEmpty(subscriptionEvent.OrganizationCode))
			{
				var emailParams = new SubscriptionEmailParams
				{
					...
					Type = SubscriptionEmailType.ExpirationInWeek,
				};
